...
Java_net_packrig_ft8native_Ft8Native_nativeVersion(JNIEnv* env, jobject) {
    // FT8LIB_SHORT_HASH comes from FT8LIB_COMMIT in CMakeLists.txt — see
    // docs/FT8_LIB_UPGRADE.md. Only the packrig-native version is bumped here.
    return env->NewStringUTF("packrig-native 0.3.0 (ft8_lib " FT8LIB_SHORT_HASH ")");
}

extern "C" JNIEXPORT jobjectArray JNICALL
Java_net_packrig_ft8native_Ft8Native_nativeDecode(
    ...
    for (int i = 0; i < numCandidates; ++i) {
        ...
        DecodeOut out;
        out.text = text;
        // SNR is computed in Kotlin (net.packrig.core.SnrEstimator); ft8_lib's
    ...
    jclass cls = env->FindClass("net/packrig/ft8native/Ft8DecodeResult");
...
Java_net_packrig_ft8native_Ft8Native_nativeEncode(
...
Java_net_packrig_ft8native_Ft8Native_nativeClearCallsignTable(JNIEnv*, jobject) {
    std::lock_guard<std::mutex> guard(g_decodeMutex);
    hashtable_init();
