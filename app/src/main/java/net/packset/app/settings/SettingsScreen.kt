...
import net.packset.core.StationProfileValidator
...
fun SettingsScreen(vm: OperateViewModel) {
    ...
    // Snackbar events surface via the app-level host in PacksetApp.
    Scaffold(
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun GeneralSettingsTab(vm: OperateViewModel, state: OperateUiState) {
    SettingsTabColumn {
        SettingsSection("Station") {
            SettingsTextField(
                persistedValue = state.myCall,
                onValueChange = vm::setMyCall,
                transform = { it.uppercase() },
                label = { Text("Callsign") },
                placeholder = { Text("N0CALL") },
                modifier = Modifier.fillMaxWidth(),
                isError = { it.isNotBlank() && !StationProfileValidator.isValidCall(it) },
                supportingText = { shown ->
                    if (shown.isNotBlank() && !StationProfileValidator.isValidCall(shown)) {
            ...
            SettingsTextField(
                persistedValue = state.myGrid,
                onValueChange = vm::setMyGrid,
                transform = { it.uppercase() },
                label = { Text("Grid") },
                placeholder = { Text("FN31") },
                modifier = Modifier.fillMaxWidth(),
                isError = { it.isNotBlank() && !StationProfileValidator.isValidGrid(it) },
                supportingText = { shown ->
                    if (shown.isNotBlank() && !StationProfileValidator.isValidGrid(shown)) {
...
@Composable
private fun RigsSettingsTab(vm: OperateViewModel, state: OperateUiState) {
    SettingsTabColumn {
...
@Composable
private fun DisplaySettingsTab(vm: OperateViewModel, state: OperateUiState) {
    SettingsTabColumn {
...
@Composable
private fun IntegrationsSettingsTab(vm: OperateViewModel, state: OperateUiState) {
    SettingsTabColumn {
        SettingsSection(
            "QRZ Logbook",
            titleBadge = {
