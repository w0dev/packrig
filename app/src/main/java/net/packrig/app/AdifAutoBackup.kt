 ...
 * a best-effort mirror in Documents/packrig via [DocumentsAdifMirror] — the
 * private copy is deleted on uninstall; the mirror survives it.
 */
object AdifAutoBackup {

    private const val TAG = "AdifAutoBackup"
    private const val FILE_NAME = "packrig-logbook.adi"
    private const val TMP_NAME = "packrig-logbook.adi.tmp"
    ...
    /** Success snackbar copy — only claims Documents/packrig when the durable mirror was written. */
    fun backupSnackbarText(mirrored: Boolean): String =
        if (mirrored) "ADIF backup written to Documents/packrig"
