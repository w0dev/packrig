...
- An amateur radio license valid for your jurisdiction and band/mode. PackRig
...
- A rig. PackRig's verified reference setups are the Yaesu FT-891 + Digirig
...
PackRig ships two release channels that install side by side on the same
device: stable (`net.packrig`) and unstable (`net.packrig.unstable`). Unstable is
...
Android needs two things before PackRig can talk to the interface: the
...
PackRig finds a device matching your rig profile and asks Android to grant
...
If you decline USB permission, or no matching device is present, PackRig falls
...
reconnecting the interface) lets PackRig bind to it without restarting the app.
...
When a USB audio interface is attached, PackRig selects it automatically — you
...
PackRig is receive-only until you turn it on. Flip **Enable transmit** in the
...
QSO on the Operate tab — PackRig shows a one-time "Confirm before transmitting"
...
Go to the **Operate** tab and tap **Start decoding**. PackRig opens the USB
